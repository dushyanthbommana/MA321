...
house_data_2=read.csv("house-data.csv",stringsAsFactors = T) #reading csv file
View(house_data_2)
dim(house_data_2)
...
split_2b= sample.split(data_2$Overallcond, SplitRatio = 0.8)
training_set_2b=subset(data_2,split_2b == TRUE)
test_set_2b=subset(data_2,split_2b == FALSE)

#Random forest on training data set

# we need to set seed to achieve same results every time.
